...
| 4 | Clean | dart_process_clean.py | Kills leaked Dart/Flutter tool-host processes hanging VS Code | D:\src\contacts\scripts\clean\dart_process_clean.py | Y |
| 5 | Clean | flutter_sdk_repair.py | Flutter SDK + Dart analyzer plugin repair (AOT/devtools/custom_lint) | D:\src\contacts\scripts\clean\flutter_sdk_repair.py | Y |
...
| 11 | Test | run_test.py | Reliable Flutter test runner for Windows (kills orphaned flutter_tester.exe) | D:\src\contacts\scripts\test\run_test.py | Y |
...
| 16 | Report | daily_report.py | JSON report of recent Git changes to Dart/JSON/YAML/assets | D:\src\contacts\scripts\report\daily_report.py | Y |
| 17 | Report | dependency_report.py | Flutter dependency report with optional upgrade capabilities | D:\src\contacts\scripts\report\dependency_report.py | Y |
...
### Batch 3: scripts #4, #5, #11, #16, #17

Five Python scripts migrated from `D:\src\contacts\scripts\` into the extension's bundled script library at `extension/scripts/library/`. Each script is a copy of the production original with one mechanical change: the `saropa_branding` import path adjusted from `.shared` to `_shared` (the shared branding module is bundled alongside at `extension/scripts/library/_shared/saropa_branding.py`). `run_test.py` additionally changes from a hardcoded `parents[2]` project root to `Path.cwd()` (the library runner sets cwd via `$workspaceRoot`) and defaults to `test/` when no args are given (makes it schedulable without interactive prompts).

**What shipped:**

| Library ID | Source | Tags | Key change |
|---|---|---|---|
| `dart-process-clean` | `clean/dart_process_clean.py` | cleanup, dart, flutter | Runs with `--yes` (no confirmation); `--dry-run` via Set Params |
| `run-test` | `test/run_test.py` | test, flutter | Defaults to `test/` when blank; uses `$workspaceRoot` cwd |
| `daily-report` | `report/daily_report.py` | report, git | Requires PyYAML (documented in description) |
| `flutter-sdk-repair` | `clean/flutter_sdk_repair.py` | cleanup, flutter, repair | Requires elevated terminal (documented) |
| `dependency-report` | `report/dependency_report.py` | report, flutter, dependencies | Requires PyYAML; tqdm optional |

**Scheduling path:** The library system does not natively carry a `schedule` field. To schedule a library script, promote it to a stored shortcut (via the Launcher's Pin action or the tree context menu), then configure a schedule on the promoted shortcut. All five scripts are self-contained and either argument-free or have sensible defaults, so the scheduler can fire them unattended after the first prompted run remembers the user's answer.

**Verification:** `npx tsc --noEmit` clean, `node esbuild.js` clean, `npm test` 1044/1044 pass.

**Remaining:** 19 of 26 scripts are not yet migrated (the plan stays active).
