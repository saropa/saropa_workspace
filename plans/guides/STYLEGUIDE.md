...
- **Pane visibility is controlled from the left panel's category list; groups are collapsible with chevrons.** Two independent levels of disclosure: which pane(s) the center grid shows is controlled by a single-select category list in the left panel (buildCategoryList(), `launcherCategoryList.ts`) — **All** plus one row per pane (My shortcuts / Recipes / Watches / Project files / Scripts / Notes / Mobile Remote Control), each showing its own live item count; selecting a row narrows the center grid to just that pane, selecting **All** shows every pane grouped as normal (PLAN_Launcher_Restructure.md build order step 3, replacing an earlier per-pane header-chip toggle design — see the plan's own build order for why). Each group inside a grouped pane folds via its own header (chevron + the group's own glyph/tint + a count) over its card grid; group collapse is stored in `store.collapsed` (keyed by group id) and persists across reloads via `getState`/`setState`. While a search query is active, a collapsed group reveals its matching cards (`.root.searching` re-displays `.group-body`, declared after the collapsed rules to win at equal specificity), so a result is never hidden behind a fold (developer feedback 2026-06-28). The header's own meta line is now purely informational (project name/version plus a scheduled-rituals count) — it has no toggles of its own.
...
- **The header's meta line is purely informational; pane visibility lives in the left panel.** `LauncherStat` (`launcherViewData.ts`) carries no pane id any more — every entry renders as a plain, non-interactive `<span class="meta-item">` (an icon + text), never a button. `buildHeader()` emits at most one stat, the scheduled-rituals count (clock icon, shown only when it is greater than zero), since scheduled cards live inside the "mine" pane and the left panel has no "narrow to scheduled" row of its own. The version chip (`.meta-item.version`) is the only other entry, shown when a manifest declares one. Which pane(s) the center grid shows is a single-select choice in the left panel's category list instead (see the "Pane visibility is controlled from the left panel's category list" bullet above) — there is no per-pane toggle, hidden state, or reset control in the header any more.
...
### 4.9a Exported documents carry l10n-resolved strings at generation time
...
### 4.13 Webview form panels support Ctrl+Enter to save and Escape to cancel

A webview that acts as a form (Customize, Configure Run, Set Params, Schedule Editor) wires `Ctrl+Enter` / `Cmd+Enter` to its Save button and `Escape` to its Cancel button via a document-level `keydown` listener in the client script. These are the same gestures VS Code uses for its own confirmation dialogs and commit message boxes, so they need no labeling or l10n. The listener delegates to the existing button `.click()` rather than duplicating the save/cancel logic.

### 4.14 An empty-state label that names its own fix is a link to that fix
...
### 4.15 A recent-items QuickPick ends with a Browse fallback
