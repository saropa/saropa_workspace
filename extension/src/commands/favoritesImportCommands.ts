...
export function registerFavoritesImportCommands(
  ...
  reg("saropaWorkspace.importFavorites", () => handleImportFavorites(store));
  reg("saropaWorkspace.scanSiblingFavorites", () => handleScanSiblingFavorites(store));
}

// Detect every favorites source this workspace has (kdcro101 files, the built-in
// favorites.resources setting, the Sabitovvt favorites panel) and import all of
// them in one pass. Names every source drawn from in the toast, and offers a
// one-click jump to the output channel when any entry was skipped, so a
// malformed favorite is never silently dropped.
async function handleImportFavorites(store: ShortcutStore): Promise<void> {
...
async function handleScanSiblingFavorites(store: ShortcutStore): Promise<void> {
