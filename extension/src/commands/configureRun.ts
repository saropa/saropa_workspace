...
// modules (configureRunCommand / configureRunEnv / configureRunMode); the hub's item
// rows and per-cluster dispatch live in ./configureRunHub (split out to keep this
// file under the project's line-count cap); this file owns the working copy, the
// hub loop, and persistence.
//
...
// "Configure Run Parameters" command entry point. Warns and exits for an auto-shortcut
// (nowhere to persist exec config), otherwise builds a working copy of the exec config
// and the top-level concurrency settings, runs the hub loop, and persists both on Save
// (concurrency is a separate store call since it does not live on ShortcutExecConfig).
export async function configureRun(store: ShortcutStore, shortcut: Shortcut): Promise<void> {
  ...
  if (!(await runConfigureHubLoop(work, conc, store, shortcut, title))) {
...
function buildWorkingCopy(shortcut: Shortcut): ShortcutExecConfig {
  return {
...
function buildConcurrencyEdit(shortcut: Shortcut): ConcurrencyEdit {
  return {
...
// Returns false on Esc (discard) or true once the user picks Save at the hub,
// leaving persistence to the caller.
async function runConfigureHubLoop(
  ...
  for (;;) {
    ...
    const signal = await applyHubChoice(choice, work, conc, store, shortcut, title);
    if (signal === "cancel") {
      return false;
    }
    if (signal === "save") {
