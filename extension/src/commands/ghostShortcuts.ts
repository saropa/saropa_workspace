...
// Bare navigation/util commands that are never worth adding even when frequent.
...
// Suggest shortcuts from frequently-typed shell commands (WOW #2): scan local
// shell history for complex commands typed at least a few times and offer to save
// the ones the user picks as global shell shortcuts. Read-only until the user
// explicitly selects — nothing is added or run automatically, and the history is
// never transmitted or modified.
export async function suggestFromHistory(store: ShortcutStore): Promise<void> {
  const MIN_COUNT = 3;
  ...
  for (const item of picked) {
    ...
    // shortcut keeps the full command (shown as the row detail and run verbatim).
    const label =
    ...
    // not a repo artifact, and a shell shortcut carries no path that would tie it to
    // one folder. Run in the integrated terminal so its output is visible.
    const ok = await store.addShellShortcut(label, item.command, "global", true);
    if (ok) {
      added++;
    }
  }
