// Unit tests for the pure pin-row text formatters (pinRowFormatting.ts). These are
// string-in / string-out label builders split out of pinTreeItem so the item class
...
// and the assertions can pin the exact rendered text.
...
import {
  ...
  actionSummary,
  urlHost,
} from "../views/pinRowFormatting";
import { Pin } from "../model/pin";
import { RunResult } from "../exec/runStatus";
import { PinBadge } from "../exec/pinBadges";

// A minimal stored pin; callers override only the fields a case cares about. Mirrors
// the helper in pinFilter.test so the shape stays familiar across the view tests.
...
test("expirySummary: a pin with no expiry has no chip", () => {
  assert.equal(expirySummary(pin({})), undefined);
});
...
test("expirySummary: a wall-clock bomb shows the time-left countdown", () => {
  ...
  assert.equal(expirySummary(pin({ expires: { at } })), "2h left");
});

test("expirySummary: a branch bomb names the branch it is tied to", () => {
  assert.equal(
    expirySummary(pin({ expires: { onBranchAway: "feature/x" } })),
...
test("expirySummary: when both conditions are set the countdown wins", () => {
  ...
  assert.equal(
    expirySummary(pin({ expires: { at, onBranchAway: "main" } })),
    "30m left"
  );
});
...
test("formatDiagTooltip: a badge with no diagnostic half yields undefined", () => {
  ...
  assert.equal(formatDiagTooltip({ testsPassed: 5 } as PinBadge), undefined);
  assert.equal(formatDiagTooltip({} as PinBadge), undefined);
});
...
test("formatDiagTooltip: present severity counts render, missing ones default to 0", () => {
  ...
  const text = formatDiagTooltip({ errors: 3 } as PinBadge);
...
test("formatTestTooltip: a badge with no test half yields undefined", () => {
  assert.equal(formatTestTooltip({ errors: 1 } as PinBadge), undefined);
  assert.equal(formatTestTooltip({} as PinBadge), undefined);
});

test("formatTestTooltip: a present tally renders, the missing side defaulting to 0", () => {
  const text = formatTestTooltip({ testsPassed: 12 } as PinBadge);
...
test("actionSummary: a file pin (no action) summarizes to its path", () => {
  assert.equal(actionSummary(pin({ path: "src/app.ts" })), "src/app.ts");
});
...
test("actionSummary: a url action shows only the host, never the full URL", () => {
  ...
  assert.equal(
    actionSummary(pin({ action: { kind: "url", url: "https://github.com/a/b?q=1" } })),
    "github.com"
  );
});
...
test("actionSummary: a shell action shows its command line, empty when unset", () => {
  assert.equal(
    actionSummary(pin({ action: { kind: "shell", shellCommand: "npm test" } })),
    "npm test"
  );
  // A shell action with no command line contributes an empty detail rather than a
  // crash or the falsy "undefined" string.
  assert.equal(actionSummary(pin({ action: { kind: "shell" } })), "");
});

test("actionSummary: a command action shows the command id, empty when unset", () => {
  assert.equal(
    actionSummary(pin({ action: { kind: "command", commandId: "editor.action.format" } })),
    "editor.action.format"
  );
  assert.equal(actionSummary(pin({ action: { kind: "command" } })), "");
});

test("actionSummary: a macro action counts its steps (0 when absent)", () => {
  assert.equal(
    actionSummary(
      pin({ action: { kind: "macro", steps: [{ kind: "open" }, { kind: "shell" }] } })
    ),
    "2 steps"
  );
  assert.equal(actionSummary(pin({ action: { kind: "macro" } })), "0 steps");
});

test("actionSummary: a routine action counts its members (0 when absent)", () => {
  assert.equal(
    actionSummary(
      pin({ action: { kind: "routine", members: [{ pinId: "p1" }] } })
    ),
    "1 recipes"
  );
  assert.equal(actionSummary(pin({ action: { kind: "routine" } })), "0 recipes");
});
...
test("actionSummary: an annotation kind falls through to the path", () => {
  ...
  assert.equal(actionSummary(pin({ path: "note", action: { kind: "comment" } })), "note");
});
