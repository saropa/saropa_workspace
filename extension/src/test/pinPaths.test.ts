// Unit tests for the global-pin path round-trip (pinPaths.ts). These touch
...
import { parseGlobalPath, globalStoredPath } from "../model/pinPaths";

test("globalStoredPath stores a local file as its plain fsPath", () => {
  // A local pin reads naturally and dedupes against older pins as a bare path.
