// Unit tests for the leaf module of the PinStore class chain (model/pinStoreShared.ts):
...
import {
  ...
  setsEqual,
  sameSetName,
} from "../model/pinStoreShared";

test("the global-state keys are namespaced and distinct", () => {
  // Pins and groups live under separate mementos; a shared key would have one
...
test("recipeSubGroupId routes a known suite key, else falls back to the base group", () => {
  ...
  // key must not strand the pin in a non-existent folder — it stays in the parent.
