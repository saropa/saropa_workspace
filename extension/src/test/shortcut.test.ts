// Unit tests for the core shortcut data model (model/shortcut.ts). This module is pure
// types plus a handful of exported helpers and constants — shortcutKind /
// isAnnotationShortcut / emptyProjectShortcutsFile and the PROJECT_SHORTCUTS_VERSION /
...
// are annotations, what kind a shortcut runs as) and the empty-file shape the store
...
// A minimal stored shortcut; callers override only the fields a case exercises. The
...
test("pinKind: a pin with no action runs as a file (the implicit Phase-1 default)", () => {
  // The whole "file" branch hinges on action being absent — every plain file shortcut
  // takes this path, so the fallback must be exactly "file".
  assert.equal(shortcutKind(shortcut({})), "file");
});
...
test("pinKind: a pin's action kind is its run kind", () => {
  ...
  assert.equal(shortcutKind(shortcut({ action: { kind: "shell" } })), "shell");
  assert.equal(shortcutKind(shortcut({ action: { kind: "url" } })), "url");
  assert.equal(shortcutKind(shortcut({ action: { kind: "command" } })), "command");
  assert.equal(shortcutKind(shortcut({ action: { kind: "macro" } })), "macro");
  assert.equal(shortcutKind(shortcut({ action: { kind: "routine" } })), "routine");
});
...
test("isAnnotationPin: only comment and separator are inert annotations", () => {
  ...
  assert.equal(isAnnotationShortcut(shortcut({ action: { kind: "comment" } })), true);
  assert.equal(isAnnotationShortcut(shortcut({ action: { kind: "separator" } })), true);
});

test("isAnnotationPin: every runnable / openable kind is NOT an annotation", () => {
  // A file shortcut (no action) and each real action must fail the predicate so they
  // keep their run/open behavior — a false positive here would silence a real shortcut.
  assert.equal(isAnnotationShortcut(shortcut({})), false);
  assert.equal(isAnnotationShortcut(shortcut({ action: { kind: "shell" } })), false);
  assert.equal(isAnnotationShortcut(shortcut({ action: { kind: "url" } })), false);
  assert.equal(isAnnotationShortcut(shortcut({ action: { kind: "command" } })), false);
  assert.equal(isAnnotationShortcut(shortcut({ action: { kind: "macro" } })), false);
  assert.equal(isAnnotationShortcut(shortcut({ action: { kind: "routine" } })), false);
});
...
test("emptyProjectPinsFile: stamps the current version and the Default set", () => {
  ...
  const file = emptyProjectShortcutsFile();
  assert.equal(file.version, PROJECT_SHORTCUTS_VERSION);
...
test("emptyProjectPinsFile: returns a fresh object each call (no shared mutable state)", () => {
  ...
  const a = emptyProjectShortcutsFile();
  const b = emptyProjectShortcutsFile();
...
test("the schema version is 3 (the named-pin-sets version)", () => {
  ...
  assert.equal(PROJECT_SHORTCUTS_VERSION, 3);
});
