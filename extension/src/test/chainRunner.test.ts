...
// force-to-background pin clone the idle path and the cross-file watch links reuse,
// and it is testable in isolation. These cases pin its two shapes (a file pin vs a
// recipe-action pin) and prove it never mutates the stored pin.
...
function filePin(over: Partial<Pin> = {}): Pin {
  return {
    id: "p1",
    path: "build.sh",
    scope: "project",
...
test("toBackground returns a clone and never mutates the stored pin", () => {
  ...
  assert.notEqual(bg, pin, "a fresh object is returned");
  assert.equal(
    pin.exec?.useIntegratedTerminal,
    true,
    "the original pin's exec is unchanged — the override applies to this run only"
  );
  assert.equal(pin.exec?.runLocation, undefined, "the stored pin keeps its own location");
});
