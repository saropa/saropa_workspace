// Unit tests for the per-shortcut diagnostic / test badge parsing and formatting (recipe
// book #26 / #32). parseRunBadge / formatBadgeLead are pure over a run's combined
// output, and the shortcutBadges registry is in-memory with a VS Code EventEmitter (modeled
...
//
// shortcutBadges is a module-level singleton, so each registry test uses its own shortcut id and
...
// --- shortcutBadges registry round-trip --------------------------------------

test("pinBadges records, reads, and clears a badge per pin", () => {
  const pinId = "pb-roundtrip";
  try {
    const badge: ShortcutBadge = { errors: 1, warnings: 2, infos: 0, at: 123 };
    shortcutBadges.record(pinId, badge);
    assert.deepEqual(shortcutBadges.get(pinId), badge, "the stored badge is read back");
    shortcutBadges.clear(pinId);
    assert.equal(shortcutBadges.get(pinId), undefined, "clear drops the badge");
  } finally {
    shortcutBadges.clear(pinId);
  }
});
...
test("pinBadges fires onDidChange on record and on a removing clear", () => {
  ...
  let fires = 0;
  const sub = shortcutBadges.onDidChange(() => fires++);
  try {
    shortcutBadges.record(pinId, { at: 1 });
    assert.equal(fires, 1, "recording fires a change");
    shortcutBadges.clear(pinId);
    assert.equal(fires, 2, "removing an existing badge fires a change");
    // Clearing a shortcut that has no badge must not fire (nothing changed).
    shortcutBadges.clear(pinId);
