// Unit tests for the foundation layer of the PinStore class chain
// (model/pinStoreBase.ts): the synchronous query accessors and the file / global-
// state IO + migration that every higher layer builds on. The split files are
// abstract internals composed into PinStore, so this drives a real PinStore against
// the fs-backed vscode stub over a temp directory — exercising readProjectFile's
// defensive defaults and v1/v2 migration, resolveUri, findPin / findPinByUri,
...
import { PROJECT_PINS_VERSION, DEFAULT_SET_NAME } from "../model/pin";
...
test("findPin resolves a pin by id across project and global scopes", async () => {
  // The click dispatcher carries only an id, so findPin must reach both scopes.
  const ctx = fakeContext();
  const store = new PinStore(ctx);
  await store.init();
  await store.addPin(asUri(Uri.joinPath(folder.uri, "proj.ts")), "project");
  await store.addPin(asUri(Uri.file(tmpDir + "/glob.ts")), "global");

  const proj = store.getProjectPins().find((p) => p.path === "proj.ts");
  const glob = store.getGlobalPins().find((p) => p.path.endsWith("glob.ts"));
  assert.ok(proj && glob);
  assert.equal(store.findPin(proj!.id)?.id, proj!.id);
  assert.equal(store.findPin(glob!.id)?.id, glob!.id);
  assert.equal(store.findPin("no-such-id"), undefined);
});
...
test("resolveUri joins a project pin to its folder and parses a global pin's path", async () => {
  ...
  await store.init();
  await store.addPin(asUri(Uri.joinPath(folder.uri, "src/a.ts")), "project");
  await store.addPin(asUri(Uri.file(tmpDir + "/g.ts")), "global");

  const proj = store.getProjectPins().find((p) => p.path === "src/a.ts")!;
  ...
  const glob = store.getGlobalPins().find((p) => p.path.endsWith("g.ts"))!;
...
test("findPinByUri locates the just-added pin by its resolved URI", async () => {
  ...
  // resolved URI of the pin, not just any path fragment.
  const store = new PinStore(fakeContext());
  await store.init();
  const target = Uri.joinPath(folder.uri, "lib/main.ts");
  await store.addPin(asUri(target), "project");
  const found = store.findPinByUri(asUri(target), "project");
...
test("tagsInUse returns the de-duplicated, sorted union of stored pin tags", async () => {
  ...
  await store.addPin(asUri(Uri.joinPath(folder.uri, "b.ts")), "project");
  const a = store.getProjectPins().find((p) => p.path === "a.ts")!;
  const b = store.getProjectPins().find((p) => p.path === "b.ts")!;
  await store.setPinTags(a, ["ops", "dev"]);
  await store.setPinTags(b, ["ops"]); // "ops" shared -> de-duplicated
...
test("readProjectFile defends a malformed file by falling back to the empty shape", async () => {
  ...
  // example pin shows (an auto pin), so there are zero stored explicit pins.
  assert.equal(
    store.getProjectPins().filter((p) => !p.isAuto).length,
    0,
...
test("a v1 file migrates on read without dropping a pin, and only a write rewrites the version", async () => {
  // v1 has no groups/activeSet/sets; the pin (no groupId) survives and reads at top
  ...
  await store.init();
  assert.ok(
    store.getProjectPins().some((p) => p.id === "v1pin" && p.path === "old.ts"),
  ...
  assert.equal(after.version, PROJECT_PINS_VERSION, "a write persists the current version");
