// Unit tests for the recompute layer (model/pinStoreRefresh.ts): refresh()/rescan()
// rebuild the cached pin/group state from the project files + global state, then the
// async missing-file stat pass and recipe seeding run off the first paint. These are
// abstract internals PinStore composes, so the tests drive a real PinStore against
...
test("the deferred stat pass flags a file pin whose target is missing on disk", async () => {
  // recomputeMissing runs off the first paint and marks a pin whose file is gone, so
  // the tree can warn instead of letting a click hit a raw "file not found".
  const store = new PinStore(fakeContext());
  await store.init();
  // Point a pin at a path that does not exist on disk.
  const ghost = Uri.joinPath(folder.uri, "ghost.ts");
  await store.addPin(asUri(ghost), "project");
  const pin = store.getProjectPins().find((p) => p.path === "ghost.ts")!;
  await flush();
  assert.equal(store.isMissing(pin.id), true, "a pin to an absent file is flagged missing");

  // A pin whose file exists is NOT flagged.
  nodeFs.writeFileSync(nodePath.join(tmpDir, "real.ts"), "x\n");
  await store.addPin(asUri(Uri.joinPath(folder.uri, "real.ts")), "project");
  const real = store.getProjectPins().find((p) => p.path === "real.ts")!;
  await flush();
...
test("a freshly created file appears as a missing pin until the file is written", async () => {
  ...
  await store.addPin(asUri(later), "project");
  const pin = store.getProjectPins().find((p) => p.path === "later.ts")!;
  await flush();
  assert.equal(store.isMissing(pin.id), true);

  nodeFs.writeFileSync(nodePath.join(tmpDir, "later.ts"), "now here\n");
  await store.rescan();
  await flush();
  assert.equal(store.isMissing(pin.id), false, "the flag clears once the file exists");
});
...
test("with recipes disabled, no recipe groups or recipe pins are published", async () => {
  ...
  assert.deepEqual(store.getRecipePins(), []);
});
