...
// Per-pin diagnostic / test badges (recipe book #26, #32). The dawn lint sweep and
// test-trend rituals write a report but, until now, left the pin itself silent. This
// registry parses a finished run's output for severity counts (error / warning /
// info) or a test pass/fail tally and records them per pin, so the tree can badge the
// pin with "what did the last sweep find" without the user opening the report.
//
...
class PinBadgeRegistry {
  private readonly byPin = new Map<string, PinBadge>();
  private readonly _onDidChange = new vscode.EventEmitter<void>();
  readonly onDidChange = this._onDidChange.event;

  record(pinId: string, badge: PinBadge): void {
    this.byPin.set(pinId, badge);
    this._onDidChange.fire();
  }

  get(pinId: string): PinBadge | undefined {
...
export const pinBadges = new PinBadgeRegistry();
...
export function parseRunBadge(output: string): PinBadge | undefined {
...
export function formatBadgeLead(badge: PinBadge): string | undefined {
  const parts: string[] = [];
  const hasDiag =
