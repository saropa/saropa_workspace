...
// Launch a run in a brand-new integrated terminal, rooted at cwd (so no `cd` is
// ever needed) and labeled with the shortcut's name. A single shared/reused
...
export function runInTerminal(
  ...
  const terminal = vscode.window.createTerminal({ name: displayName, env, cwd });
  terminal.show(true);
