...
// with "\n". The assembly itself is further split into per-concern helpers below (the
// full sequence is over 100 lines even as one phase) so each stays under the
// project's function-length cap.
...
// followed by the target/action line. Tooltip shows the full target (the complete URL
// for a url shortcut), even though the row only shows the host — the hover is where the
// detail belongs. A masked shortcut replaces the target line with a generic notice: the
// hover is a passive surface that can sit on a shared screen, so it must never carry
// the real path. The real name is named only at the deliberate reveal confirm (see
// openShortcut).
function buildTooltipHeaderLines(input: ShortcutTooltipInput): string[] {
  const { shortcut, masked, isFile, resolvedUri } = input;
  ...
  // leak).
  return shortcut.description && !masked
    ? [shortcut.description, targetLine]
    : [targetLine];
}

// The current-state lines: what the shortcut is doing right now (running/stopping/
// paused/next scheduled run), then the notices that explain why it might be blocked
// or worth a second look (missing target, recipe click hint, dependency lock, time-
// bomb expiry). Suppressed while running/stopping where live state already answers
// the question these notices exist to answer.
function buildTooltipStatusLines(
  input: ShortcutTooltipInput,
  nextLabel: string | undefined
): string[] {
  const { shortcut, isRunning, isStopping, missing, lockedBy } = input;
  const lines: string[] = [];
  if (isStopping) {
    lines.push(l10n("run.stoppingTooltip"));
  } else if (isRunning) {
    lines.push(l10n("run.runningTooltip"));
  } else if (shortcut.paused) {
    lines.push(l10n("pause.tooltip"));
  } else if (nextLabel) {
    lines.push(l10n("schedule.nextRun", { time: nextLabel }));
  }
  ...
  if (isFileMissing(missing, isRunning, isStopping)) {
    lines.push(l10n("pin.missingTooltip"));
  }
  ...
  if (shortcut.isRecipe && !isRunning && !isStopping) {
    lines.push(l10n("recipe.clickHint"));
  }
  ...
  if (shortcut.expires && !isRunning && !isStopping) {
    if (shortcut.expires.at !== undefined) {
      lines.push(
        l10n("expiry.tooltip.at", { when: formatExpiryInstant(shortcut.expires.at) })
      );
    }
    if (shortcut.expires.onBranchAway !== undefined) {
      lines.push(
        l10n("expiry.tooltip.branch", { branch: shortcut.expires.onBranchAway })
      );
    }
  }
  return lines;
...
// True only while resting (no run in progress) with the shortcut's target gone.
// Named as "file" missing for readability, but there is no separate isFile check
// here: `missing` is store.isMissing(shortcut.id), and recomputeMissing only ever
// adds a file-kind shortcut's id to that set (see shortcutStoreRefresh.ts), so
// `missing` can never be true for a non-file shortcut in the first place.
function isFileMissing(missing: boolean, isRunning: boolean, isStopping: boolean): boolean {
  return missing && !isRunning && !isStopping;
}

// The outcome lines: the most recent run's result, the last lint/test sweep's
...
function buildTooltipOutcomeLines(input: ShortcutTooltipInput): string[] {
  ...
  if (lastRun) {
    lines.push(formatRunTooltip(lastRun));
  }
  ...
  if (runCount > 0) {
    lines.push(l10n("run.countTooltip", { count: runCount }));
  }
  return lines;
}

// The metadata + footer lines: classification tags, branch link, live metric, the
...
function buildTooltipMetadataLines(input: ShortcutTooltipInput): string[] {
  ...
  if (shortcut.branch !== undefined) {
    lines.push(l10n("branch.tooltip", { branch: shortcut.branch }));
  }
  ...
  if (untapped && !isRunning && !isStopping) {
    lines.push(l10n("untapped.rowTooltip"));
  }
  ...
  if (!shortcut.isRecipe && !isRunning && !isStopping) {
    lines.push(l10n("pin.gestureHint"));
  }
  return lines;
}
...
export function buildShortcutTooltipLines(input: ShortcutTooltipInput): string[] {
  ...
  // builders.
  const next = input.shortcut.schedule
    ? nextOccurrence(input.shortcut.schedule, Date.now())
    : undefined;
  const nextLabel = next !== undefined ? formatNextRun(next) : undefined;

  return [
    ...buildTooltipHeaderLines(input),
    ...buildTooltipStatusLines(input, nextLabel),
    ...buildTooltipOutcomeLines(input),
    ...buildTooltipMetadataLines(input),
  ];
}
