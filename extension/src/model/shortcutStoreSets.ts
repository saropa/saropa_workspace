...
import { ShortcutStoreMutation } from "./shortcutStoreMutation";

// Named shortcut-set layer: switch / create / rename / delete / duplicate the per-
// folder shortcut sets, and the file helpers that activate or clone a set's contents.
export abstract class ShortcutStoreSets extends ShortcutStoreMutation {
  // --- shortcut sets -----------------------------------------------------
  //
  // A shortcut set is a named, switchable collection of the user's project shortcuts
  ...
  // inactive sets live in ProjectShortcutsFile.sets. Sets are coordinated across a
  ...
  // an empty one where a folder has never seen that name). Global shortcuts are not
  // part of any set: a global favorite is cross-workspace by definition, so it stays
  // shared across all sets. Auto/recipe seeding is likewise workspace-level and
  // shared (it lives on ProjectShortcutsFile, not ShortcutSet).
  ...
  // The stored (explicit) project shortcuts of a named set WITHOUT switching to it,
  // read from the first workspace folder's file. The active set's shortcuts are
  // already at the file's top level; an inactive set's shortcuts live in `sets`. Used
  // by the branch-set binder's link command to offer an on-switch shortcut from the
  // set being linked (which may be inactive, so its shortcuts are not in the
  // projectShortcuts cache). Returns [] when no folder is open or the name is unknown.
  // Excludes auto/recipe shortcuts by nature — those are recomputed, never stored in a
  // set, so a file read yields only the user's explicit shortcuts (a meaningful run
  // target).
  async getSetShortcuts(name: string): Promise<Shortcut[]> {
  ...
  // Switch every folder to the set named `name`, repainting the tree to its shortcuts.
  ...
  // folder is open (project shortcut sets need a folder to live in).
  ...
  // Delete a set across all folders. Deleting a set drops its project shortcuts (a
  ...
  // Duplicate a set's shortcuts + groups under a new name and switch to it. The copy
  // is fully independent: contents are deep-cloned and given fresh ids (a shared id
  // could let an edit in one set leak into the other). Intra-set trigger /
  // dependsOn links reference shortcut ids, which are regenerated and not remapped, so
  ...
  // shortcuts/groups into `sets` under its name, then hoist the target set's
  // shortcuts/groups to the top level (an empty set when the folder has never seen the
  // name). Mutates `file` in place; the caller persists. Keeps exactly one copy of
  // each name across {active, sets}. Precondition: file.activeSet !== target.
  protected activateSetInFile(file: ProjectShortcutsFile, target: string): void {
    const incoming = file.sets.find((s) => s.name === target);
    const outgoing: ShortcutSet = {
      name: file.activeSet,
      pins: file.pins,
      groups: file.groups,
    };
  ...
  // Deep-clone a set's shortcuts + groups with fresh ids for a duplicate. Groups get
  // new ids and each shortcut's groupId is remapped to the cloned group, so the copy's
  ...
  protected cloneSetContents(
  ...
  ): { pins: Shortcut[]; groups: ShortcutGroup[] } {
    const groupIdMap = new Map<string, string>();
    const clonedGroups = (JSON.parse(JSON.stringify(groups)) as ShortcutGroup[]).map(
      (g) => {
    ...
    const clonedShortcuts = (JSON.parse(JSON.stringify(shortcuts)) as Shortcut[]).map((p) => {
      p.id = this.newId();
      if (p.groupId) {
    ...
    return { pins: clonedShortcuts, groups: clonedGroups };
  }
}
