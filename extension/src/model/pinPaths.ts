...
// Global-pin path round-trip helpers. A global pin stores either a plain absolute
// fsPath (the common local-file case) or, for a file on a non-local filesystem
// (Remote-SSH, WSL, dev container, or a virtual provider), the full URI string.
// Split out from pinStore so the file/non-file BRANCH logic is unit-testable
// against a minimal Uri stub without loading the whole store graph (roadmap 4.1).

// Resolve a global pin's stored target back to a URI. The two stored forms are told
// apart by a scheme separator: a real URI always carries "<scheme>://", while a
