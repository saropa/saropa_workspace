...
export abstract class PinStoreBase {
  protected readonly _onDidChange = new vscode.EventEmitter<void>();
  readonly onDidChange = this._onDidChange.event;

  // Cached, ready-to-render results recomputed by refresh().
  protected projectPins: Pin[] = [];
  protected globalPins: Pin[] = [];
  protected projectGroups: PinGroup[] = [];
  protected globalGroups: PinGroup[] = [];
  ...
  protected recipeGroups: PinGroup[] = [];
  ...
  protected missingPinIds = new Set<string>();
  ...
  protected projectPinFolder = new Map<string, vscode.WorkspaceFolder>();
  ...
  protected ensureGroupId(groups: PinGroup[], label: string): string {
  ...
  protected async writeGlobalPins(pins: Pin[]): Promise<void> {
    await this.context.globalState.update(GLOBAL_STATE_KEY, pins);
  }

  protected readGlobalGroups(): PinGroup[] {
    return this.context.globalState.get<PinGroup[]>(GLOBAL_GROUPS_KEY, []);
  }

  protected async writeGlobalGroups(groups: PinGroup[]): Promise<void> {
