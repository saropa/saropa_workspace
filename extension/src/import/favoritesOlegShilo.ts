...
// in isolation; the importer turns each parsed entry into a project shortcut and owns
...
//   - "file"      a path / alias line that becomes a file shortcut
...
// caller turns each entry into a shortcut and owns dedup. A duplicate file line still
// counts as a real entry for divider placement (the shortcut already exists in the
// list, so a following blank is a genuine gap), so dedup is deliberately the caller's
// job.
...
// each entry into a project shortcut in source order. File shortcuts resolve against
// the owning folder and dedupe by path (so re-import stays idempotent for real
// shortcuts); comment / separator annotations are positional and intentionally NOT
// deduped, mirroring the shortcut-set import carve-out in commands/pinSetExport.ts, so
// they re-add per source entry. Annotations target detected.folder so they land in the
// same folder and order as the file shortcuts they sit between (the no-anchor
// addAnnotationShortcut path otherwise defaults to the first folder).
export async function importOlegShilo(
  ...
  for (const entry of parseOlegShiloLines(text)) {
    ...
    if (entry.kind === "comment") {
      if (
        await store.addAnnotationShortcut("comment", "project", entry.text, undefined, detected.folder)
      ) {
        added++;
      }
      continue;
    ...
    if (entry.kind === "separator") {
      if (
        await store.addAnnotationShortcut("separator", "project", undefined, undefined, detected.folder)
      ) {
        added++;
      }
      continue;
