...
// src/test/bookmarksImport.test.ts
...
var MIN_MEMBERS = 2;
function detectRoutineRecipes(detected2) {
  const present = new Set(detected2.map((r) => r.recipeId));
...
// src/import/favoritesKdcroBookmarks.ts
var path2 = __toESM(require("path"));
var BOOKMARKS_ROOTPATH_TOKEN = "$ROOTPATH$";
function resolveBookmarkUri(rawPath, folder2) {
  let p = rawPath.trim().replace(/\\/g, "/");
  if (p.startsWith(BOOKMARKS_ROOTPATH_TOKEN)) {
    p = p.slice(BOOKMARKS_ROOTPATH_TOKEN.length).replace(/^[/\\]/, "");
  }
  return path2.isAbsolute(p) ? Uri.file(p) : Uri.joinPath(folder2.uri, p);
}
async function importBookmarks(text, detected2, store, channel) {
  let doc;
  try {
  ...
  const seen = new Set(
    store.getProjectPins().filter((p) => p.line !== void 0).map((p) => `${store.resolveUri(p)?.toString() ?? ""}#${p.line}`)
  );
  let added = 0;
  let skipped = 0;
  for (const file of doc.files ?? []) {
    if (!file.path || file.path.trim().length === 0) {
      channel.appendLine(l10n("import.log.skipNoPath", { file: detected2.fileName }));
      skipped++;
      continue;
    }
    const uri = resolveBookmarkUri(file.path, detected2.folder);
    ...
    for (const mark of file.bookmarks ?? []) {
      ...
      const label = mark.label && mark.label.trim().length > 0 ? mark.label.trim() : l10n("linePin.label", { name: path2.basename(uri.fsPath), line: pinLine });
...
(0, import_node_test.beforeEach)(() => {
  ...
  tmpDir = nodeFs2.mkdtempSync(nodePath2.join(os2.tmpdir(), "sw-bookmarks-")).replace(/\\/g, "/");
...
(0, import_node_test.test)("a bookmark imports as a line pin with the 0-based line converted to 1-based", async () => {
  ...
  import_strict.default.equal(result.added, 1, "the bookmark imports");
  import_strict.default.equal(result.skipped, 0, "nothing is skipped");
  const pin = store.getProjectPins().find((p) => p.line !== void 0);
  import_strict.default.ok(pin, "a line pin is created");
  import_strict.default.equal(pin.line, 10, "the 0-based line 9 became the 1-based line 10");
  import_strict.default.equal(pin.path, "src/app.ts", "the path is stored folder-relative");
  import_strict.default.equal(pin.label, "entry point", "the bookmark label becomes the pin label");
});
(0, import_node_test.test)("the legacy $ROOTPATH$ prefix is stripped before resolving the path", async () => {
  const store = new PinStore(fakeContext());
  await store.init();
  const json = JSON.stringify({
  ...
  const pin = store.getProjectPins().find((p) => p.line !== void 0);
  import_strict.default.ok(pin, "a line pin is created");
  import_strict.default.equal(pin.path, "lib/util.ts", "the $ROOTPATH$ token and its separator are stripped");
  import_strict.default.equal(pin.label, "util.ts:1", "the label falls back to basename:line (line 0 -> 1)");
});
(0, import_node_test.test)("a bookmark file outside any workspace folder is reported and skipped", async () => {
  const store = new PinStore(fakeContext());
  await store.init();
  const outside = `${os2.tmpdir().replace(/\\/g, "/")}/elsewhere/foreign.ts`;
  const json = JSON.stringify({
    files: [{ path: outside, bookmarks: [{ line: 2 }] }]
  });
  const { channel, lines } = fakeChannel();
  const result = await importBookmarks(json, detected(), store, channel);
  import_strict.default.equal(result.added, 0, "nothing imports from outside the folder");
  import_strict.default.equal(result.skipped, 1, "the outside-folder file is skipped");
  import_strict.default.equal(lines.length, 1, "the skip is reported to the channel");
  import_strict.default.equal(
    store.getProjectPins().filter((p) => p.line !== void 0).length,
    0,
    "no line pin is created"
  );
});
(0, import_node_test.test)("re-importing the same bookmarks adds no duplicate line pins (idempotent)", async () => {
  const store = new PinStore(fakeContext());
  await store.init();
  const json = JSON.stringify({
    files: [
  ...
  import_strict.default.equal(first.added, 2, "both distinct lines import");
  const second = await importBookmarks(json, detected(), store, channel);
  import_strict.default.equal(second.added, 0, "the second import adds nothing");
  const linePins = store.getProjectPins().filter((p) => p.line !== void 0);
  import_strict.default.equal(linePins.length, 2, "exactly the two line pins exist");
...
(0, import_node_test.test)("a malformed bookmarks file imports nothing, logs the error, and does not throw", async () => {
  const store = new PinStore(fakeContext());
  await store.init();
  const { channel, lines } = fakeChannel();
  const result = await importBookmarks("{ not valid json", detected(), store, channel);
  import_strict.default.deepEqual(result, { added: 0, skipped: 0 });
  import_strict.default.equal(
    store.getProjectPins().filter((p) => p.line !== void 0).length,
    0,
    "no line pin is created"
  );
  import_strict.default.equal(lines.length, 1, "the parse failure is reported once");
});
